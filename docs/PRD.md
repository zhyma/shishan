...
| 项目 | 内容 |
| --- | --- |
| 文档状态 | v0.2，Phase 0–2 技术基线已实现，待产品评审 |
| 日期 | 2026-08-30 |
| 产品阶段 | 四语言本地 MVP 技术验证 |
...
| 原生 Tree-sitter 安装失败 | 跨平台采用受阻 | Phase 0 比较 Node binding 与 Wasm；CI 覆盖三大系统 |
...
### Phase 2：四语言 MVP（技术实现完成，远端 CI 待分支推送）
...
## 22. 进入 Phase 3 前需要对齐的产品决策

Phase 0–2 技术基线完成后，需要确认：

1. v1 是否接受“完整叙事内嵌源码”的方案；
2. `@shishan` 行式协议的可读性是否符合预期；
3. Web 第一屏更偏“项目/函数大纲”还是“流程图”；
4. 第一种正式支持的 AI Skill 面向哪个平台；
...
1. 评审 Phase 0–2 的真实 Web 体验和注释密度；
2. 将实现分支推送并确认 Linux、macOS、Windows CI；
3. 选择一个真实中型仓库进行首次扫描与持续编辑试用；
4. 用 5–10 个 AI 编码任务评估 Skill 的同步率和噪声；
5. 根据试用结果冻结 `shishan/v1`；
6. 决定 Phase 3 优先做 diff 过期检测、编辑器薄集成还是静态分享。
